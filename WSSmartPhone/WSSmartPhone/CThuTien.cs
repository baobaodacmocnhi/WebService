...
namespace WSSmartPhone
{
    class CThuTien
    {
        ...
        {
            if (bool.Parse(_cDAL_DocSo.ExecuteQuery_ReturnOneValue("select case when exists(select NgayDoc from Lich_DocSo ds,Lich_DocSo_ChiTiet dsct where CheckNgayDoc=0 and ds.Nam=" + Nam + " and ds.Ky=" + Ky + " and dsct.IDDot=" + Dot + ") then 'true' else 'false' end").ToString()) == true)
                return true;
            else
