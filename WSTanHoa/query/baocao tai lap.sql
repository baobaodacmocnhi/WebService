select phong.Name,SLnhap=count(tc.ID),SLxuly=count(ls.ID),SLQuaHan=SUM(CASE WHEN CAST(tc.NgayBatDau as date)<CAST(DATEADD(day,-1,ls.CreateDate) as date) THEN 1 ELSE 0 END)
,SLTon=count(tc.ID)-count(ls.ID)
...
and cast(tc.CreateDate as date)>='20231001' and cast(tc.CreateDate as date)<='20231029'
group by phong.name
