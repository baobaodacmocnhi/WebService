            ...
            if (action == "Xem" && TuNgay != "" && DenNgay != "")
            {
                    ...
                    if (radLoai == "radNgayGiaiTrach")
                {
                    sql = "select Bank = bank.NGANHANG, giaitrach.SoLuong,giaitrach.TongCong from NGANHANG bank left join"
                           ...
                           + " and MaNV_DangNgan is not null and MaNH is not null"
                    ...
                    if (dt.Rows.Count > 0)
                        for (int i = 0; i < rowCount; i++)
                        {
                            ...
                            {
                                zv.SoLuong += en.SoLuong = int.Parse(dt.Rows[i]["SoLuong"].ToString());
                            }
                            if (dt.Rows[i]["TongCong"].ToString() != "")
                            {
                                zv.TongCong += en.TongCong = decimal.Parse(dt.Rows[i]["TongCong"].ToString());
                            }
                            zv.lst.Add(en);
                        }
                }
